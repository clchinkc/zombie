...
def gaussian_sampling(space_size, goal, alpha=0.5):
    if np.random.rand() > alpha:
        ...
        return np.random.normal(goal[0], std_dev), np.random.normal(goal[1], std_dev)
...
def bridge_sampling(space_size, obstacles, bridge_length=5):
    while True:
        ...
        if not is_collision(Point(point_a), Point(point_b), obstacles):
            return midpoint
...
# Choose the sampling method: 'random', 'gaussian', 'bridge', or 'informed'
sampling_method = 'informed'
