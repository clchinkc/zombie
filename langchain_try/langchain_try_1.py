...
Here is some context about the time and location of the movie:
Time: {time}
Location: {location}

Screenwriter: This is a synopsis for the above movie:
"""
prompt_template = PromptTemplate(input_variables=["title", 'one_liner', "time", "location"], template=template)
...
for p_info in prompt_infos:
    ...
    chain = LLMChain(llm=llm, prompt=prompt, output_key="refined_synopsis")
    destination_chains[name] = chain

# Create a default chain for fallback
default_chain = ConversationChain(llm=llm, output_key="refined_synopsis")
...
memory = SimpleMemory(memories={"time": "Year 2020", "location": "New York City"})

overall_chain = SequentialChain(
    chains=[synopsis_chain, review_chain, refine_synopsis_chain],
    input_variables=["title", "one_liner"],
    output_variables=["synopsis", "review", "refined_synopsis"],
    memory=memory,
    verbose=True)
