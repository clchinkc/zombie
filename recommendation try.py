...
"""
The Netflix recommendation system is a Python project that utilizes machine learning techniques, such as cosine similarity and feature engineering, to recommend movies and TV shows to users based on their input.

The project uses a variety of modules including tkinter for the graphical user interface (GUI), pandas and numpy for data cleaning and processing, and nltk for natural language processing.

The dataset containing Netflix movies and TV shows is downloaded and cleaned using pandas and numpy, and features such as cast, director, ratings, country, and genres are extracted to build the recommendation engine.

The cosine similarity approach is then used to find similar results based on user input. Finally, tkinter is used to create an interactive GUI that allows users to input their preferred movie or TV show and receive recommendations based on their preferences.

Overall, this project provides an excellent opportunity to explore and learn about machine learning features and cosine similarity in the context of recommendation systems, while also demonstrating how to build an intuitive user interface using tkinter.
"""
...
head = tk.Label(
    window, text='Enter Movie / TV Show on Netflix For Recommendations', font=('Calibri 15'))
head.pack(pady=20)
