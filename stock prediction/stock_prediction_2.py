...
def metropolis_hastings(likelihood_func, proposal_sampler, last_price, num_iterations):
    ...
    for i in range(0, num_iterations):
        ...
        # Add the accepted parameter value to the chain
...
def likelihood(final_price, previous_price, mu, sigma, theta, randomness=np.random.normal()):
    ...
    price_trajectory = previous_price + drift + diffusion
    # Calculate the likelihood of the final price
    ln_final_price = np.log(final_price)
    ln_price_trajectory = np.log(price_trajectory)
    likelihood = lognorm.pdf(ln_final_price, s=sigma, scale=np.exp(ln_price_trajectory)) + 1e-10
    return likelihood

# normal distribution with jump component
def likelihood(final_price, previous_price, mu, sigma, theta, intensity=0.1, jump_mean=0.0001, jump_sd=0.01):
    delta_t = 1  # time interval
    drift = mu * previous_price * delta_t
    diffusion = sigma * previous_price * np.sqrt(delta_t) * np.random.normal()

    # Add jump component of the jump diffusion process
    jump_count = np.random.poisson(intensity * delta_t)  # Number of jumps in the current time step
    jump_sizes = np.random.normal(jump_mean, jump_sd, jump_count)  # Jump sizes
    jump_total = np.sum(jump_sizes)  # Total jump effect

    price_trajectory = previous_price + drift + diffusion + previous_price * jump_total
...
def price_prediction(last_price, days, num_simulations):
    ...
    for d in range(days):
        ...
        # choose a random price from the chain as the starting price for the next day
        last_price = np.random.choice(chain)
    return price_matrix
...
Convergence diagnostics: To ensure that the MCMC chain has converged to the target distribution, you can use various convergence diagnostics, such as the Gelman-Rubin diagnostic or the Effective Sample Size (ESS). Monitoring these diagnostics will help you determine whether you need to make adjustments to the proposal distribution or the number of iterations.
